...
- **The `streams.http_events` field inside bundle JSONs recorded before v2.0
  can contain stale entries.** Two capture-stage defects, both fixed in v2.0:
  the bundle writer took the *oldest* five entries of a newest-first list, and
  the session-start reset cleared `streams.http` and `streams.net` but not
  `streams.http_events`. When state from an earlier session was still present,
  that session's events could appear in a later session's bundles, carrying
  their original, earlier timestamps. The scope is exactly that one field:
  `streams.http` and `streams.net` were reset correctly; the hash chain, the
  frame hashes, both timestamp mechanisms and every verification result are
  unaffected; and the published playback sessions on mncourtfraud.com were
  checked individually and are clean. The one affected session found
  (Fraud-Reports, in `examples/`) had its playback stream rebuilt from
  `streams.http` with `postprocess/build_http_streams.py`, and its
  `http_events` report is omitted from the example for that reason. When
  reading an older session's bundles, treat `streams.http_events` entries whose
  timestamps precede the session start as artefacts of this bug.
